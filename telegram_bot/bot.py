...
def main_menu_handler(message):
    if message.text == 'Set Configuration':
        markup = generate_buttons(['sampling_rate', 'use_counter', 'used_offset', 'tare_timeout', 'danger_threshold', 'danger_counter', 'back'], markup)
...
def set_config(message):
    if message == 'sampling_rate':
        requests.post(f'http://{API_URI}/api/settings/v1', headers={"Authorization": login()}, data={"sampling_rate": extract_number(message.text)})
        markup = types.ReplyKeyboardMarkup(row_width=2)
        markup = generate_buttons(['Back'], markup)
        message = bot.reply_to(message, "sampling_rate was changed", reply_markup=markup)
        bot.register_next_step_handler(message, main_menu_handler)
    elif message == 'use_counter':
        requests.post(f'http://{API_URI}/api/settings/v1', headers={"Authorization": login()}, data={"use_counter": extract_number(message.text)})
        markup = types.ReplyKeyboardMarkup(row_width=2)
        markup = generate_buttons(['Back'], markup)
        message = bot.reply_to(message, "use_counter was changed", reply_markup=markup)
        bot.register_next_step_handler(message, main_menu_handler)
    elif message == 'used_offset':
        requests.post(f'http://{API_URI}/api/settings/v1', headers={"Authorization": login()}, data={"used_offset": extract_number(message.text)})
        markup = types.ReplyKeyboardMarkup(row_width=2)
        markup = generate_buttons(['Back'], markup)
        message = bot.reply_to(message, "used_offset was changed", reply_markup=markup)
        bot.register_next_step_handler(message, main_menu_handler)
    elif message == 'tare_timeout':
        requests.post(f'http://{API_URI}/api/settings/v1', headers={"Authorization": login()}, data={"tare_timeout": extract_number(message.text)})
        markup = types.ReplyKeyboardMarkup(row_width=2)
        markup = generate_buttons(['Back'], markup)
        message = bot.reply_to(message, "tare_timeout was changed", reply_markup=markup)
        bot.register_next_step_handler(message, main_menu_handler)
    elif message == 'danger_threshold':
        requests.post(f'http://{API_URI}/api/settings/v1', headers={"Authorization": login()}, data={"danger_threshold": extract_number(message.text)})
        markup = types.ReplyKeyboardMarkup(row_width=2)
        markup = generate_buttons(['Back'], markup)
        message = bot.reply_to(message, "danger_threshold was changed", reply_markup=markup)
        bot.register_next_step_handler(message, main_menu_handler)
    elif message == 'danger_counter':
        requests.post(f'http://{API_URI}/api/settings/v1', headers={"Authorization": login()}, data={"danger_counter": extract_number(message.text)})
        markup = types.ReplyKeyboardMarkup(row_width=2)
        markup = generate_buttons(['Back'], markup)
